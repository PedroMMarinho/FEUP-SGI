...
export class CowboyChair extends THREE.Object3D {
	...
	build() {
		...
		fl_legMesh.position.set(- (this.xxLength / 2 - this.legOffset) + this.xpos, this.ypos - this.legHeight/2, -(this.zzLength / 2 - this.legOffset) + this.zpos);
		fr_legMesh.position.set((this.xxLength / 2 - this.legOffset) + this.xpos, this.ypos - this.legHeight/2, -(this.zzLength / 2 - this.legOffset) + this.zpos);
		bl_legMesh.position.set(- (this.xxLength / 2 - this.legOffset) + this.xpos, this.ypos - this.legHeight/2, (this.zzLength / 2 - this.legOffset) + this.zpos + this.thickness);
		br_legMesh.position.set((this.xxLength / 2 - this.legOffset) + this.xpos, this.ypos - this.legHeight/2, (this.zzLength / 2 - this.legOffset) + this.zpos + this.thickness);
		bl_legMesh.rotation.x = br_legMesh.rotation.x = - Math.PI * this.backlegRotationFac;

		let legPosY = this.ypos - this.legHeight / 2;

		b_crossbarMesh.position.set(this.xpos, legPosY, (this.zzLength / 2) + this.zpos + 0.1);
		r_crossbarMesh.position.set(this.xpos + this.xxLength / 2 - 0.2, legPosY, this.zzLength / 4 + this.zpos - this.thickness);
		l_crossbarMesh.position.set(this.xpos - this.xxLength / 2 + 0.2, legPosY, this.zzLength / 4 + this.zpos - this.thickness);
