...
class SeaweedGroup extends THREE.Object3D {
    ...
    createMaterial() {
        this.seaweedTexture = this.textureManager.getTexture('seaweed');
        this.seaweedNormalMap = this.textureManager.getTexture('seaweed-normal');
        
        // Ensure texture wrapping
        if (this.seaweedTexture) {
            this.seaweedTexture.wrapS = THREE.RepeatWrapping;
            this.seaweedTexture.wrapT = THREE.RepeatWrapping;
        }

        if(this.seaweedNormalMap){
            this.seaweedNormalMap.wrapS = this.seaweedNormalMap.wrapT = THREE.RepeatWrapping
        }
        ...
        const material = new THREE.MeshPhongMaterial({
            ...
            onBeforeCompile: (shader) => {
                ...
                shader.uniforms.uTexture = { value: this.seaweedTexture };
                shader.uniforms.uNormalMap = { value: this.seaweedNormalMap };
                shader.uniforms.uTextureScale = { value: 0.5 }; // Adjusted scale
                shader.uniforms.uNormalScale = { value: 1.0 }; // Normal map strength
                shader.uniforms.uUseTexture = { value: this.seaweedTexture ? 1.0 : 0.0 };
                shader.uniforms.uUseNormal = { value: this.seaweedNormalMap ? 1.0 : 0.0 };
